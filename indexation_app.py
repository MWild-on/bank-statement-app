...
def fmt_money(value: Decimal | float | int) -> str:
    ...
    # формат 123456.78 -> 123 456.78 руб.
    s = f"{value:.2f}"
    parts = s.split(".")
...
def compute_indexation_for_period(
    ...
    while True:
        ...
        if key not in cpi_map:
            raise ValueError(f"Нет ИПЦ для {y}-{m:02d} — добавь этот месяц на лист 'Индекс'.")
...
def calculate_indexation_for_debt(
    ...
    if pays.empty:
        ...
        if order_date <= cutoff_date:
            ind = compute_indexation_for_period(initial_debt, order_date, cutoff_date, cpi_map)
            return ind, [{
...
def generate_pdf_bytes_for_debt(
    ...
    # Заголовок
    draw_line(f"Расчёт индексации присуждённой денежной суммы. Рег. номер {reg_num}", bold=True)
    draw_line("")
    draw_line(f"Крайняя дата расчёта: {fmt_date(cutoff_date)}")
    draw_line(
        f"Сумма долга на дату вынесения приказа ({fmt_date(order_date)}): {fmt_money(base_debt)}"
    )
...
def process_workbook(uploaded_file, cutoff_date: dt.date):
    ...
    for idx, row in main_df.iterrows():
        ...
        if periods:
    		...
    		periods=periods,
    		cutoff_date=effective_cutoff,   # <-- вот эту строку не забудь
...
def main():
    st.set_page_config(page_title="Расчёт индексации", layout="wide")
